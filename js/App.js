var App = new function() {
	...
	this.populateGameList = function(myGames) {
		...
		$.each(myGames, function(index, game) {
			...
			$('#gamelist tbody').append('<tr><td>' + game._id + '</td><td>' + game.enemyName + '</td>SATUS<td>' + game.status +
				'</td><td><button class="btn btn-success play-game" data-gameId="' + game._id + '">PLAY</button></td></tr>');
		});
	...
	this.populateShipList = function(ships) {
		...
		$(ships).each(function(index, ship) {
			...
			$('.shiplist tbody').append('<tr><td width="200" class="ship' + $(this)[0]._id + 'label">' + $(this)[0].name +
				'</td><td width="100" data-id="' + $(this)[0]._id +
				'"><img data-type="horizontal" src="images/glyphicons-212-right-arrow.png" class="boat" /><img data-type="vertical" src="images/glyphicons-213-down-arrow.png" class="boat" /></div><div class="resetButton" style="display: none"><button class="btn btn-danger resetShip">Reset</button></div></td><td><div class="boatLength">' +
				$(this)[0].length + '</div></td></tr>');
	...
	this.loopCoords = function(coord, length, isVertical) {
		...
		for (var i = 0; i < length; i++) {
			...
			if (isVertical) {
				...
				shipCoords.push({
					'x': String.fromCharCode(coord.x.charCodeAt(0) + i),
					'y': coord.y
				});
	...
	this.drawShips = function(game) {
		...
		$.each(game.myGameboard.ships, function(index, ship) {
			...
			for (var i = 0; i < ship.length; i++) {
				...
				if (ship.isVertical) {
					App.fillCell('#myGameboard', {
						'x': ship.startCell.x,
						'y': ship.startCell.y + i
					}, color);
				} else {
					App.fillCell('#myGameboard', {
						'x': String.fromCharCode(ship.startCell.x.charCodeAt(0) + i),
						'y': ship.startCell.y
					}, color);
				}
			}
...
$(document).ready(function() {
	...
	$('#placeBoard').click(function() {
		if (App.checkBoard()) {
			...
			alert('Je moet eerst de booten plaatsen');
		}
	...
	$('.shipPanel').on('click', '.resetShip', function() {
		...
		$(this).closest('td').html(
			'<img data-type="horizontal" src="images/glyphicons-212-right-arrow.png" class="boat" /><img data-type="vertical" src="images/glyphicons-213-down-arrow.png" class="boat" /></div><div class="resetButton" style="display: none"><button class="btn btn-danger resetShip">Reset</button></div>'
		);
		$('.boat').draggable({
			revert: true
		})
